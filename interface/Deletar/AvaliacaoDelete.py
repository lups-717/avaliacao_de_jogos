...
def delete_Avaliacao(id_valor):
    url = f'http://localhost:5000/api/avaliacoes/{id_valor}'
...
def deletar_avaliacao():
    root = tk.Tk()
    root.title("Exclusão de Avaliações")
    ...
    tk.Label(frame, text="ID da Avaliação:").grid(row=9, column=0, sticky="w")
    delete_id_entry = ttk.Entry(frame, width=25)
    delete_id_entry.grid(row=9, column=1, padx=5, pady=5)

    # Botão para excluir avaliação
    excluir_btn = ttk.Button(frame, text="Excluir", command=lambda:delete_Avaliacao(delete_id_entry.get()))
